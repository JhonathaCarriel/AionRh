...
import { CadastroPageModule } from 'src/app/ncms/cadastro/cadastro.module';
...
export class CriarPage implements OnInit {
  ...
  constructor(
    ...
    private loggerService: LoggerService // Adicione esta linha
...
private prepararDadosParaInsercao(): any {
  ...
  formData.consumidorFinal = formData.consumidorFinal || 'Não';
  formData.produtoImportado = formData.produtoImportado || 'Não';
  formData.produtoEstoque = formData.produtoEstoque || 'Não';
  ...
  for (const name in this.simuladorForm.controls) {
    if (this.simuladorForm.controls[name].invalid) {
      ...
      if (label.includes('Fornecedor')) {
        ...
        label = label.replace('Cliente', '').trim();
      }
